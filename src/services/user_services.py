...
from schemas.token import UserCreate, UserResponse, TokenResponse
...
class UserService:
    ...
    async def signup(self, user_details: UserCreate) -> UserResponse:
        user_exists = await self._user_repository.check_user_exists_by_email(email=user_details.email)
        if user_exists:
    ...
    async def login(self, login_details: UserCreate) -> TokenResponse:
        ...
        if not verify_password(password=login_details.password, hashed_password=user.password_hash):
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid credentials")
